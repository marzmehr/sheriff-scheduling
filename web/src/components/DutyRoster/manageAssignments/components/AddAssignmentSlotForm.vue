                    ...
                    <b-td >
                        <b-tr class="bg-white">
                            <b-form-group style="margin: 0.25rem 0 0 0.5rem;width: 15rem">                            
                                <label class="h6 m-0 p-0">Duty<span class="text-danger">*</span></label>
                                <b-form-select 
                                    size="sm"
...
<script lang="ts">
    ...
    export default class AddAssignmentSlotForm extends Vue {        
        ...
        originalDuty = {} as manageAssignmentDutyInfoType | undefined;       
        originalStartTime = '';
        originalEndTime = '';
        originalNote = '';
        errorMsg = ''
        ...
        public extractFormInfo(){           
            ...
            const index = this.dutyList.findIndex(duty=>{if(duty.id == this.formData.id)return true})
            this.originalDuty = this.selectedDuty = (index>=0)? this.dutyList[index]: {} as manageAssignmentDutyInfoType;
            this.originalStartTime = this.selectedStartTime = this.formData.startTime?this.formData.startTime:'';            
        ...
        public saveForm(){
            ...
            if(!this.selectedDuty || (this.selectedDuty && !this.selectedDuty.dutyId) ){
                this.dutyState  = false;
                requiredError = true;            
            } else {
                this.dutyState  = true;
            }
...
//                 if(sheriffId == this.formData.sheriffId){
...
//                 this.$emit('submit',this.selectedDuty?this.selectedDuty.sheriffId:0, allEditingDutySlots , false);  
//             }
        }

        public validateTime(selectedTimeBins){
            //TODO: add functionality to validate time and prevent conflicts
        ...
        public confirmedAssignOverTimeDuty(){
            ...
            this.$emit('submit',this.selectedDuty?this.selectedDuty.dutyId:0, this.editedDutySlots , false);
        }
        ...
        public isChanged(){            
            if((this.originalDuty && this.selectedDuty && (this.originalDuty.dutyId != this.selectedDuty.dutyId)) ||
        ...
        public clearSelections(){
            this.selectedDuty = {} as manageAssignmentDutyInfoType;
