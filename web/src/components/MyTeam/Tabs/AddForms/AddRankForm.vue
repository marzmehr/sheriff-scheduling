    ...
    <div>
                            ...
                            <b-form-group style="margin: 0.05rem 0 0 0.5rem;width: 17rem"> 
                                <b-form-select
                                    ...
                                    :state = "rankState?null:false">
                                        ...
                                        <b-form-select-option
                                            v-for="rank in commonInfo.sheriffRankList" 
                                            :key="rank.id"
                                            :value="rank">
                                                {{rank.name}}
                                        </b-form-select-option>     
        ...
        <b-modal v-model="showCancelWarning" id="bv-modal-rank-cancel-warning" header-class="bg-warning text-light">            
            <template v-slot:modal-title>
                <h2 v-if="isCreate" class="mb-0 text-light"> Unsaved New Rank </h2>                
                <h2 v-else class="mb-0 text-light"> Unsaved Rank Changes </h2>                                 
            </template>
...
<script lang="ts">
    ...
    import {commonInfoType, sheriffRankInfoType} from '../../../../types/common';
    ...
    export default class AddRankForm extends Vue {
        ...
        @Prop({required: true})
        formData!: tempRankInfoType;

        @Prop({required: true})
        isCreate!: boolean;

        selectedRank = {} as sheriffRankInfoType;
        rankState = true;      
        ...
        originalRank = {} as sheriffRankInfoType;
        originalStartDate = '';
        originalEndDate = '';
        originalStartTime = '';
        originalEndTime = '';
        ...
        public extractFormInfo(){
            ...
            const index = this.commonInfo.sheriffRankList.findIndex(rank=>{if(rank.id == this.formData.rankId)return true})
            this.originalRank = this.selectedRank = (index>=0)? this.commonInfo.sheriffRankList[index]: {} as sheriffRankInfoType;
        ...
        public saveForm(){
                ...
                if(this.selectedRank && !this.selectedRank.id ){
                    ...
                    const body = {
                        rankId: this.selectedRank?this.selectedRank.id:0,
                        startDate: startDate,
                        endDate: endDate,                      
                        isFullDay: isFullDay,
                        id: this.formDataId,
        ...
        public isChanged(){
            if(this.isCreate){
                ...
                if((this.originalRank && this.selectedRank && (this.originalRank.id != this.selectedRank.id)) ||
        ...
        public clearSelections(){
            this.selectedRank = {} as sheriffRankInfoType;
